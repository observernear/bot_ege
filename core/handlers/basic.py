...
async def make_test_message(message: Message, bot: Bot, state: FSMContext):
    ...
    for i in range(len(problems)):
        try:
            if i % 2 == 0:
                if (subject == 'math' and int(problems[i]) > 19) or (subject == 'rus' and int(problems[i]) > 27) or (subject == 'inf' and int(problems[i]) > 27) or (subject == 'phys' and int(problems[i]) > 26) or (subject == 'en' and int(problems[i]) > 42) or (subject == 'soc' and int(problems[i]) > 25):
                    raise Exception
